        ...
        <div class="bg-white overflow-auto">
            <table class="text-left w-full border-collapse"> <!--Border collapse doesn't work on this site yet but it's available in newer tailwind versions -->
                ...
                <tbody>
                    ...
                    @foreach ($data as $dt)
                    @php
                        $pekerja = \App\Models\User::where('id', $dt->user_id)->first();
                    @endphp
                        <tr class="hover:bg-grey-lighter">
                            <td class="py-4 px-6 border-b border-grey-light">{{$no++}}</td>
                            <td class="py-4 px-6 border-b border-grey-light">{{ $pekerja->name }}</td>
                            ...
                            <td class="py-4 px-6 border-b border-grey-light">
                                @if(Auth::user()->role_id == 3)
                                    <a class="px-4 py-1 text-white font-light tracking-wider bg-blue-900 rounded" href="jadwal/{{$dt->id}}">Laporan</a>
                                @endif
                                @if(Auth::user()->role_id == 1)
